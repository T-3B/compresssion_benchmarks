...
TIMEFORMAT=%R
LC_ALL=C  # for . instead of , as digit sep for `time`

jpgopt() {
	local b c i=$2 k=$3 a=$4
	b=$(./ssimulacra2 "$i" /dev/shm/"$1"_"$i"$k.jpg)
...
jpgliXYB() {  # cjxl v0.10.2 e1489592 [AVX2,SSE4,SSE2]
	jpgopt cjpegliXYB "$2" $1 $({ time ./cjpegli "$2" /dev/shm/cjpegliXYB_"$2"$1.jpg --xyb -q $1 --quiet &>/dev/null;} 2>&1)
}

parallel --jl parallel.log --bar --resume --memsuspend 3G ::: guli\ {84..100..4} moz\ {10..100..5} turbo\ {10..100..5} jpgli\ {10..100..5}\ {444,440,422,420} jpgliXYB\ {10..100..5} ::: "$@" >>jpgbench.csv || exit $?
grep +ECT, jpgbench.csv >>jpgbench_ect.csv
grep +ultrascan, jpgbench.csv >>jpgbench_ultrascan.csv
grep -ve +ECT, -e +ultrascan, jpgbench.csv >jpgbench2.csv
mv jpgbench{2,}.csv
